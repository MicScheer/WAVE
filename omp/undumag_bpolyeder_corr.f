*CMZ :  4.00/07 05/08/2020  09.43.38  by  Michael Scheer
...
*KEND.
     ...
     &  kfail(nthreadp),kinsidelocal(nthreadp)
      ...
      if (iomp.ne.0) then
        call undumag_bpolyeder_corr_omp(xin,yin,zin,bxout,byout,bzout,ifail)
        return
      endif

      if (magmag.gt.0) then
        print*,"*** Error: Call to undumag_bpolyeder_corr in undumag_bpolyeder_corr!"
        stop
      endif
...
c                print*,"undumag_bpolyeder_corr:",imag,iplan,bpetm(1,7,iplan,imag)
...
c                      endif !bcvn
     ...
     &                blab(3).ne.blab(3)
     &                ) then
                    print*,"*** Error 3 in undumag_bpolyeder_corr: blab is not a number (NaN) ***"
                    print*,
