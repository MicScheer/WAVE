*CMZ :  4.00/11 26/02/2021  17.57.38  by  Michael Scheer
...
*-- Author :    Michael Scheer   08/03/2018
     ...
     &  xsymmm
      ...
      integer istat,i,ical,lunfil,j,nallo
      ...
      if (ical.eq.0.and.kwave.eq.0) then
        ...
        if (nwind+nrace+ncrace+narc+ncarc+nthwir+nrbar.gt.0) then
          ...
          do i=1,nwind
            if (wind(1,i).eq.0.0d0) cycle
            call undumag_wind_to_fila(i)
          enddo

          do i=1,nrace
            if (race(1,i).eq.0.0d0) cycle
            call undumag_race_to_fila(i)
          enddo

          do i=1,ncrace
            if (crace(1,i).eq.0.0d0) cycle
            call undumag_crace_to_fila(i)
          enddo

          do i=1,narc
            if (arc(1,i).eq.0.0d0) cycle
            call undumag_arc_to_fila(i)
          enddo

          do i=1,nrbar
            if (rectbar(1,i).eq.0.0d0) cycle
            call undumag_bar_to_fila(i)
          enddo

          do i=1,ncarc
            if (carc(1,i).eq.0.0d0) cycle
            call undumag_circ_arc_to_fila(i)
          enddo

          do i=1,nthwir
            if (thickwire(1,i).eq.0.0d0) cycle
            call undumag_thwire_to_fila(i)
          enddo
...
c concept of wires:
        ...
        do i=1,ncwires
          write(lunfil,'(i5,7g15.5,2i5)')
     &      nint(wire(1,i)),wire(2:8,i),nint(wire(9,i)),nint(wire(10,i))
        enddo
        close(lunfil)
