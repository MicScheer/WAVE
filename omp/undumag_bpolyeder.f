*CMZ :  4.00/07 05/08/2020  11.36.57  by  Michael Scheer
...
*KEND.
     ...
     &  kfail(nthreadp),kinsidelocal(nthreadp)
      ...
      if (iomp.ne.0) then
        call undumag_bpolyeder_omp(xin,yin,zin,bxout,byout,bzout,ifail)
        return
      endif
...
c                      if (iseqdebug.ne.0) print*,xin,iplan,icorn,bplan,kwarn
     ...
     &                blab(3).ne.blab(3)
     &                ) then
                    print*,"*** Error 3 in undumag_bpolyeder: blab is not a number (NaN) ***"
                    print*,
     ...
     &  bxp,byp,bzp,ifailp)
      ...
      if (ifailm.eq.0.and.ifailp.eq.0) then
        ...
        if (ifailin.ge.0) then
          print*, "*** Warning in undumag_bpolyeder: Could not recover for x,y,z:",
