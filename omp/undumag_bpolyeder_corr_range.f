*CMZ :  4.00/07 05/08/2020  17.30.33  by  Michael Scheer
...
*-- Author :    Michael Scheer   02/12/2003
      subroutine undumag_bpolyeder_corr_range(magi,mage,xin,yin,zin,bxout,byout,bzout,ifail)
c
...
*KEND.
     ...
     &  kfail(1),kinsidelocal(1)
      ...
      integer nmaxth,ith,magi,mage

      save

      data ical/0/,nmaxth/1/,ith/1/

      if (magmag.gt.0) then
        print*,"*** Error: Call to undumag_bpolyeder_corr_range in undumag_bpolyeder_corr_range!"
        stop
      endif
...
c                print*,"undumag_bpolyeder_corr_range:",imag,iplan,bpetm(1,7,iplan,imag)
...
c                if (iseqdebug.ne.0) print*,"bcvn:",bcvn
...
c                      if (bplan(1).ne.bplan(1)) then
c                        print*,"qx,bcvn:",qx,bcvn
c                      endif
c                      if (bplan(2).ne.bplan(2)) then
c                        print*,"qy,bcvn:",qy,bcvn
c                      endif
c                      if (bplan(3).ne.bplan(3)) then
c                        print*,"qz,bcvn:",qz,bcvn
c                      endif
c                      endif !bcvn
     ...
     &                blab(3).ne.blab(3)
     &                ) then
                    print*,"*** Error 3 in undumag_bpolyeder_corr_range: blab is not a number (NaN) ***"
                    print*,
...
c                  print*,"x,dlab:",xx,dlab
c                  print*,"vnormlablab:",vnormlab
                  if( dlab(1)*vnormlab(1)+dlab(2)*vnormlab(2)+
     &                dlab(3)*vnormlab(3).gt.0.d0) then
                    iout=1
                    goto 911
