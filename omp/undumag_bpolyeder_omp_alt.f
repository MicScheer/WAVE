*CMZ :  4.00/07 05/08/2020  11.51.37  by  Michael Scheer
...
*-- Author :    Michael Scheer   02/12/2003
      subroutine undumag_bpolyeder_omp_alt(xin,yin,zin,bxout,byout,bzout,ifail)
c
...
c        print*,"Number of CPU cores used:",nmaxth
        bo=0.0d0
        ical=1
      endif
...
!$OMP& SHARED(kfail,iseqdebug,kinsidelocal,ical,bpetm,window,bpebc,bpemag,ibpeplan,ibpecorn,bperot,bo,iwarnbound,nwarnbound) FIRSTPRIVATE(nmag,itiny,jtiny,iwtiny,tiny,xx,yy,zz,kwarni,kwarn)
...
c                      if (iseqdebug.ne.0) print*,xin,iplan,icorn,bplan,kwarn
     ...
     &                blab(3).ne.blab(3)
     &                ) then
                    print*,"*** Error 3 in undumag_bpolyeder_omp: blab is not a number (NaN) ***"
                    print*,
     &                "ith,imag,iplan,xx,yy,zz:",ith,imag,iplan,xx,yy,zz,iseqdebug
c                    print*,"blab",blab
c                    print*,"tsinv",tsinv
                    kfail(ith)=imag
                    stop
...
c      if (iseqdebug.ne.0) print*,"ifail:",ifail
...
C Not working for unknown reasons for OMP ?? still true 20.4.2017??
      ...
      call undumag_bpolyeder_corr_omp(xin-corrtiny,yin-corrtiny,zin-corrtiny,
     &  bxm,bym,bzm,ifailm)
