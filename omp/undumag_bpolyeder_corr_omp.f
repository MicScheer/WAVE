*CMZ :  4.00/07 06/08/2020  10.45.22  by  Michael Scheer
...
*-- Author :    Michael Scheer   02/12/2003
      subroutine undumag_bpolyeder_corr_omp(xin,yin,zin,bxout,byout,bzout,ifail)
c
...
*KEND.
     ...
     &  kfail(nthreadp),kinsidelocal(nthreadp)
      ...
      if (magmag.gt.0) then
        print*,"*** Error: Call to undumag_bpolyeder_corr_omp in undumag_bpolyeder_corr_omp!"
        stop
      endif
...
c      if (xin.eq.0.0d0) return
      ...
      if (ical.eq.0) then
        ...
        print*,"Number of CPU cores used:",nmaxth
        bo=0.0d0
        ical=1
      endif
...
c                print*,"undumag_bpolyeder_corr_omp:",imag,iplan,bpetm(1,7,iplan,imag)
...
c                      endif !bcvn
     ...
     &                blab(3).ne.blab(3)
     &                ) then
                    print*,"*** Error 3 in undumag_bpolyeder_corr_omp: blab is not a number (NaN) ***"
                    print*,
