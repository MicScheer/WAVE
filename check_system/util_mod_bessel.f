...
*-- Author :    Michael Scheer   10/05/2012
        subroutine util_mod_bessel(v,x,besk)
c
c Aus dem Internet:
...
c*** Funktioniert nicht, nur für x>=1. bei v=5/3
...
cmsh        WRITE(*,*)'  Please enter v, x '
cmsh        READ(*,*)V,X
        N=INT(V)
cmsh        V0=V-N
cmsh        WRITE(*,30)V0,X
cmsh        IF (N.LE.10) THEN
cmsh           NS=1
cmsh        ELSE
cmsh           WRITE(*,*)'  Please enter order step Ns'
cmsh           READ(*,*)NS
cmsh        ENDIF
        CALL IKV(V,X,VM,BI,DI,BK,DK)
cmsh        NM=INT(VM)
cmsh        WRITE(*,*)
cmsh        WRITE(*,*)'    n       Iv(x)           Iv''(x)          ',
cmsh     &            'Kv(x)           Kv''(x)'
cmsh        WRITE(*,*)'  -------------------------------------',
cmsh     &            '--------------------------------'
cmsh        DO 10 K=0,NM,NS
cmsh10         WRITE(*,20) K,BI(K),DI(K),BK(K),DK(K)
cmsh20      FORMAT(2X,I4,1X,4D16.8)
cmsh30      FORMAT(8X,'v = n+v0',',   ','v0 =',F7.5,',   ','x =',F5.1)
        besk=bk(n)
        return
        END
