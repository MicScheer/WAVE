*CMZ : 00.00/14 22/09/2011  12.06.33  by  Michael Scheer
*-- Author :    Michael Scheer   19/09/2011
      subroutine util_c_spline_1d(n,xa,ya,xknots,coef,deriv,ws1,iws2,
...
c mode: <0 new initialization
     ...
     &  ws1(7*n),x,y,yp,ypp,yint,dspps1
      ...
      if (mode.lt.0) then
        ...
        if (ifail.ne.0) then
          cerr='*** Error in util_c_spline_1d: Bad return from DSPIN1 ***'
          return
        endif
        call dspcd1(ndeg,nknots,nderiv,xknots,coef,deriv,ifail)
        if (ifail.ne.0) then
          cerr='*** Error in util_c_spline_1d: Bad return from DSPCD1 ***'
          return
        endif
      endif
      ...
      if (ifail.ne.0) then
        cerr='*** Error in util_c_spline_1d: Bad return from DSPPS1 for y-value ***'
        return
      endif
      nderiv=1
      yp   = dspps1(ndeg,nknots, nderiv,x,xknots,deriv,ifail) !derivative
      if (ifail.ne.0) then
        cerr='*** Error in util_c_spline_1d: Bad return from DSPPS1 for first derivative ***'
        return
      endif
      nderiv=1
      ypp   = dspps1(ndeg,nknots, nderiv,x,xknots,deriv,ifail) !derivative
      if (ifail.ne.0) then
        cerr='*** Error in util_c_spline_1d: Bad return from DSPPS1 for second derivative ***'
        return
      endif
      nderiv=-1
      yint = dspps1(ndeg,nknots, nderiv,x,xknots,coef,ifail) !integral
      if (ifail.ne.0) then
        cerr='*** Error in util_c_spline_1d: Bad return from DSPPS1 for integral ***'
        return
      endif
