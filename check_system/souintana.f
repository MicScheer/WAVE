...
*KEND.
     ...
     &  ,DMODU,DMODU0,DDMODU,AX,AY,AZ,AX0,AY0,AZ0
...
c     &  ,H2,H2R2
     ...
     &  ,are(3),aim(3),yp2zp2i,robsv,phiobsv,
...
C--- LOOP OVER ALL FREQUENCES
     ...
     &          RARG(ICOMP)/BET1N/OM*EXPOM*(ZONE-DEXPOMPH)*DEXPbunch/sqnphsp
            affe(icomp,ifrob)=affe(icomp,ifrob)+daff
          ENDDO   !ICOMP
     ...
     &      *R2/(sqrt(R2)-ampr2corr/2.0d0)**2/nelec
      ...
      DO kfreq=1,NFREQ
        ...
        IF (IAMPLI.LT.0) THEN
          ...
          affe(1,ifrob)=(0.0D0,0.0D0)
          affe(2,ifrob)=(0.0D0,0.0D0)
          affe(3,ifrob)=(0.0D0,0.0D0)
...
c            slope=sqrt(vyelec**2+vzelec**2)/vxelec
     ...
     &          APOLL*CONJG(APOLL)
        ...
        if (ibunphase.eq.1) then
          ...
          are(1:3)=dreal(affe(1:3,ifrob))
          aim(1:3)=dimag(affe(1:3,ifrob))
          ...
          if (iobsv.eq.nobsv.and.kfreq.eq.nfreq) then
            ...
            do jfreq=1,nfreq
              ...
              are(1:3)=dreal(affe(1:3,jfrob))
              aim(1:3)=dimag(affe(1:3,jfrob))
            ...
            do job=1,nobsv
              do jfreq=1,nfreq
                ...
                affe(1:3,jfrob)=dcmplx(
     &            azcos(jfreq)*are+azsin(jfreq)*aim,
     &            -azsin(jfreq)*are+azcos(jfreq)*aim
     &            )

                AFREQ(1,jfrob)=AFREQ(1,jfrob)+affe(1,jfrob)
                AFREQ(2,jfrob)=AFREQ(2,jfrob)+affe(2,jfrob)
                AFREQ(3,jfrob)=AFREQ(3,jfrob)+affe(3,jfrob)

                affe(1,jfrob)=(0.0D0,0.0D0)
                affe(2,jfrob)=(0.0D0,0.0D0)
                affe(3,jfrob)=(0.0D0,0.0D0)
