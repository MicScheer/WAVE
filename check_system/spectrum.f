...
C----   OBSERVATION POINTS TO BE TREATED
     ...
     &         '     ',SNGL(WSIGZ(ISIGSTO)),SNGL(WSIGY(ISIGSTO))
      ...
      ALLOCATE(REAIMA(5,2,NOBSV*NFREQ))
      DO I=1,NOBSV*NFREQ
        REAIMA(1,1,I)=0.D0
        REAIMA(1,2,I)=0.D0
        REAIMA(2,1,I)=0.D0
      ...
      IF (MPINR.NE.0) THEN
        ALLOCATE(REAIMARPHI(5,2,NOBSV*NFREQ))
        DO I=1,NOBSV*NFREQ
          REAIMARPHI(1,1,I)=0.D0
          REAIMARPHI(1,2,I)=0.D0
          REAIMARPHI(2,1,I)=0.D0
          REAIMARPHI(2,2,I)=0.D0
          REAIMARPHI(3,1,I)=0.D0
          REAIMARPHI(3,2,I)=0.D0
          REAIMARPHI(4,1,I)=0.D0
          REAIMARPHI(4,2,I)=0.D0
          REAIMARPHI(5,1,I)=0.D0
          REAIMARPHI(5,2,I)=0.D0
        ENDDO
      ENDIF
...
C--- LOOP OVER ALL SOURCES, EVALUATE INTEGRALS
      ...
      IF (IAMPSKIP.NE.0) THEN
        ALLOCATE(AFREQ(3,NOBSV*NFREQ))
        AFREQ=(0.0D0,0.0D0)
        IF (MPINR.GT.0) THEN
          ALLOCATE(AFREQRPHI(3,NOBSVRPHI*NFREQ))
...
1357    CONTINUE
        ...
        if (kampli.ne.0.and.ibunch.eq.0) then
          nbunch=mbuncho
          neinbunch=meinbuncho
          ibunch=ibuncho
          ihbunch=ihbuncho
          !if (mampthreads.eq.0) then
          !  call amprep
          !else
c          if (ibunch.ne.0) then
c            if (iamppin.eq.3.or.nobsv.lt.nbunch*neinbunch) then
c            else
c              call amprep_nobsv_omp
c            endif
c          else
c            call amprep_nobsv_omp
c          endif
