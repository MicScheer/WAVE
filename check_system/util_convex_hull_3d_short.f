*CMZ :          27/09/2017  08.54.30  by  Michael Scheer
*CMZ : 00.00/21 13/03/2017  11.10.32  by  Michael Scheer
...
*-- Author :    Michael Scheer   22/11/2016
      subroutine util_convex_hull_3d_short(npoi,x,y,z,khull,kedge,kface,
     ...
     &  p1(3),p2(3),p3(3),pp(3),pt1(3),pt2(3),pt3(3),
...
c      double precision, dimension (:,:), allocatable ::  vnpl
     ...
     &  nh2d,nedgeheap,
     &  nbuff,nbuffp,nbuffm,iface,nfpoi,kfail,kfacelast,nb,k,
     &  ib1,ib2
      ...
      ical=ical+1
      tiny2=tiny**2
c      print*,"util_convex_hull_3d_short:",ical

c      allocate(vnpl(3,2*npoi))
...
c      do ipoi=1,npoi
c        write(99,*)x(ipoi),y(ipoi),z(ipoi),ipoi
c      enddo
...
c        z2(3)=p3(3)
c
c        call util_weed_line(3,x2,y2,z2,nh1d,kbuff)
c
c        if (nh1d.lt.3) then
c          cycle
c        endif
...
c        if (nbuffm.ge.3) exit
      ...
      if (nbuff.ne.3) then
        print*, "*** Error 1 in util_convex_hull_3d_short: No face found!"
        kfail=-3
        goto 9999
      endif
...
c      lface=lface+nh2d
      ...
      do while (nedgeheap.gt.0)
        ...
        if (mface.le.0.or.kedge(4,medge).ne.0) then
          print*,"*** Error 2 in util_convex_hull_3d_short ***"
          goto 9999
        endif
...
c        nedgeheap=nedgeheap-1
     ...
     &          ) then
              ...
              if (kedge(3,ledge).eq.0) then
                print*,"*** Error in util_convex_hull_3d_short: Bad edge ***"
                kfail=kfail+1
              endif
      ...
      if (kfail.eq.0) then
        ...
        if (lface-1.ne.kfacelast) then
          kfail=kfail+1
          print*,"*** Error in util_convex_hull_3d_short: Bad kfacelast ***"
        endif
