...
c     &          *GAMGAM0/GAMGAM+AMPDT
     ...
     &        isub.eq.neinbunch
     &        ) then
            are(1:3)=dreal(afferphi(1:3,ifrob))
            aim(1:3)=dimag(afferphi(1:3,ifrob))
     ...
     &    ) then
        ...
        DO IOBSV=1,NOBSVRPHI
          ...
          if (ibunphase.eq.1) then
            ...
            afferphi(1:3,ifrob)=dcmplx(
     &        azcos(ifreq)*are+azsin(ifreq)*aim,
     &        -azsin(ifreq)*are+azcos(ifreq)*aim
     &        )
          endif

          unphrphi(1:3,ifrob)=unphrphi(1:3,ifrob)+afferphi(1:3,ifrob)
...
cold            endif
            ...
            afreqrphi(1:3,ifrob)=afreqrphi(1:3,ifrob)
     &        +unphrphi(1:3,ifrob)
cold     &        *sqbunnor
...
cold      if (ielec.ne.nelec) return
      ...
      do iobsv=1,nobsv
        ...
        do ifreq=1,nfreq
          ...
          afreq(1,ifrob)=afreq(1,ifrob)*reflec(1)
          afreq(2,ifrob)=afreq(2,ifrob)*reflec(2)
          afreq(3,ifrob)=afreq(3,ifrob)*reflec(3)
...
c only used for Ntuple 5700 so far, 28.3.2012
        ...
        do iobsv=1,nobsvrphi
          ...
          do ifreq=1,nfreq
            ...
            if(speccut.gt.0.0d0) then
              if(freq(ifreq).gt.speccut*ecdipev1*dmyenergy**2*ecmaxs) then
                afreqrphi(1,ifrob)=(0.0d0,0.0d0)
                afreqrphi(2,ifrob)=(0.0d0,0.0d0)
                afreqrphi(3,ifrob)=(0.0d0,0.0d0)
              endif
            endif
     ...
     &          )*specnor*bunnor
              ...
              reaimarphi(1,1,iobfr)=reaimarphi(1,1,iobfr)+
     &          dreal(afreqrphi(1,ifrob))
     &          /sqnbunch
              reaimarphi(2,1,iobfr)=reaimarphi(2,1,iobfr)+
     &          dreal(afreqrphi(2,ifrob))
     &          /sqnbunch
              reaimarphi(3,1,iobfr)=reaimarphi(3,1,iobfr)+
     &          dreal(afreqrphi(3,ifrob))
     &          /sqnbunch

              reaimarphi(1,2,iobfr)=reaimarphi(1,2,iobfr)+
     &          dimag(afreqrphi(1,ifrob))
     &          /sqnbunch
              reaimarphi(2,2,iobfr)=reaimarphi(2,2,iobfr)+
     &          dimag(afreqrphi(2,ifrob))
     &          /sqnbunch
              reaimarphi(3,2,iobfr)=reaimarphi(3,2,iobfr)+
     &          dimag(afreqrphi(3,ifrob))
     &          /sqnbunch
