*CMZ :          04/05/2023  09.45.44  by  Michael Scheer
...
c     WRITE(6,*)'READING NAMELIST PHASEN'
     ...
     &  bunchcharge=echarge1*neinbunch
      ...
      IF (IAMPLI.NE.0.or.ibunch.ne.0) THEN
        READ(LUNGFI,AMPLIN)
        IF (IAMPLI.LT.0) IAMPSKIP=0
      ENDIF
...
C}--- CONVERT B-FIELD TO MAP
      ...
      if (iundulator.eq.2.and.ispec.eq.0) iundulator=1
...
c          NLPOI=-9999
     ...
     &        '      *** Warning in GFINIT: IEFOLD is used for IBUNCH, be careful!'
          ...
          IF (IPIN.GT.0.and.ipin.ne.3.and.neinbunch.eq.1) then
            IPIN=3
            WRITE(6,*)' '
            WRITE(6,*)
...
C--- WRITE CONTROL FLAGS
     ...
     &                             IERZANA,IERZFUN,I2DIM
      ...
      if (ibunch.ne.0.and.mthreads.ne.0.and.icluster.eq.0) then
        write(6,*)""
