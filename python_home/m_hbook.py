...
#import mhb_to_pylist as w
#from mhb_to_pylist import *
...
#Hdir.append([Cdir,Ndir,Nh1,Nh2,Nntup,Nnctup,H1head,H1,H2head,H2,Nhead,Ntup,Nctup,Nind])
...
fcfg = ''
if WavesMode == 'WAVES': fcfg = 'waveplot.cfg'
...
def debug(kmenu=None,kitem=None):
  ...
  Debug,FWAVEIN,FWVS,Wavein,WaveinO,Menu,Mapping,MenuVeto,MenuAllVeto,NMenuAllVeto,\
...
def get_geo_all():
    ...
    wid,h,x,y = getgeo()
    swid = Wmaster.winfo_screenwidth()
    shei = Wmaster.winfo_screenheight()
    ...
    return swid,shei,wid,h,canw,canh,x,y,bbox,axw,axh
#enddef get_geo_all()
...
def read_window_geometry(fname='ntupplot.cfg'):
  ...
  if WavesMode == 'WAVES': fname = 'waveplot.cfg'
...
#*CMZ :          28/09/2019  14.39.13  by  Michael Scheer
  ...
  if Waveplot == 1: #WAVE
    ...
    srun = "Run " + str(Wrun)
    ftext = plt.gcf()
    if not Trun or iforce: Trun = ftext.text(x,y,srun,fontsize=fontsize)
