*CMZ :          25/09/2026  11.18.12  by  Michael Scheer
*CMZ :  4.02/01 19/08/2026  18.33.46  by  Michael Scheer
...
*-- Author :    Michael Scheer   27/03/2019
      subroutine winstgenphoefold(kgenphoerr)
...
*KEND.
     ...
     &  g(ngenphoefold+1),gsum,be(1000),bw(1000)
      ...
      integer igphrun,ipos(2,ngenphoefold),kgenphoerr,npola,kpola,iz,iy,itz,ity,ifrq,kfrq,iwcode,
     &  lz,ly,ltz,lty,lpola,isour,nsource

      integer :: iline=0,kempty,kgenpho,kgenphodum
...
*KEND.
      ...
      if (kellip.ne.0.and.nharmell.ne.0) then
        write(6,*)
        write(6,*)'*** WARNING IN WINSTGENPHOEFOLD: KELLIP.NE.0.AND.NHARMELL.NE.0!'
      ...
      if (KHALBA.ne.0.and.NHHALBA.ne.0) then
        write(6,*)
        write(6,*)'*** WARNING IN WINSTGENPHOEFOLD: KHALBA.NE.0.AND.NHHALBA.NE.0!'
        write(6,*)'*** This means that for all beam energies the undulator field is different!'
        write(6,*)'*** This is only useful for test purposes!'
        write(6,*)
      endif
      ...
      if (KHALBASY.ne.0.and.NHHALBASY.ne.0) then
        write(6,*)
        write(6,*)'*** WARNING IN WINSTGENPHOEFOLD: KHALBASY.NE.0.AND.NHHALBASY.NE.0!'
        write(6,*)'*** This means that for all beam energies the undulator field is different!'
        write(6,*)'*** This is only useful for test purposes!'
        write(6,*)
      endif

      open(newunit=lunwef,file="genpho.gef")

      kgenphoerr=0

      ebeammin=dmyenergy*(1.0d0-nsige*espread)
      ebeammax=dmyenergy*(1.0d0+nsige*espread)
      debeam=(ebeammax-ebeammin)/dble(ngenphoefold-1)
      ...
      gsum=0.0d0
      do kgenpho=1,ngenphoefold
        g(kgenpho)=exp(-((ebeammin+(kgenpho-1)*debeam-dmyenergy)/deltae)**2/2.0d0)/sqrt(twopi1)/deltae
        gsum=gsum+g(kgenpho)
      enddo
      if (ngenphoefold.le.3.or.nosplineef.ne.0) then
        g=g/gsum
        gsum=1.0d0
      endif
      ...
      do kgenpho=1,ngenphoefold
        ...
        write(cline,*)'Winstgenphoefold:',kgenpho,' of ',ngenphoefold
...
99      close(lunout)
        ...
        cstage=trim(chwavedir)//"/.stage.genpho"
        ...
        if (istat.ne.0) then
          print*,"*** Error winstgenphoefold: Can't create sub-directory"
          print*,trim(cstage)
        endif
